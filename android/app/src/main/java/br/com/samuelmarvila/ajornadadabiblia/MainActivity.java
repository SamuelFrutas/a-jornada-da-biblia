...
public class MainActivity extends Activity {
    ...
    private void addStatsRow(LinearLayout content){
        ...
        for(int i=0;i<3;i++){LinearLayout c=card();c.setPadding(dp(10),dp(12),dp(10),dp(10));TextView v=text(values[i],22,true);v.setGravity(Gravity.CENTER);v.setTextColor(GREEN);c.addView(v);TextView l=text(labels[i],10.5f,false);l.setGravity(Gravity.CENTER);l.setTextColor(MUTED);c.addView(l);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,-2,1);lp.setMargins(i==0?0:dp(5),0,i==2?0:dp(5),dp(14));row.addView(c,lp);}content.addView(row);
    }
    ...
    private void renderCalendar(LinearLayout cal){
        TextView month=null;GridLayout grid=null;for(int i=0;i<cal.getChildCount();i++){View v=cal.getChildAt(i);if(v instanceof LinearLayout && i==2){LinearLayout nav=(LinearLayout)v;View center=nav.getChildAt(1);if(center instanceof TextView)month=(TextView)center;}if(v instanceof GridLayout)grid=(GridLayout)v;}if(month!=null)month.setText(monthFormat.format(calendarCursor.getTime()));if(grid!=null)renderCalendarInto(cal,grid,month);
    }
    private void renderCalendarInto(LinearLayout cal,GridLayout grid,TextView month){
        ...
        for(int day=1;day<=max;day++){Calendar c=(Calendar)calendarCursor.clone();c.set(Calendar.DAY_OF_MONTH,day);String key=dateKeyFormat.format(c.getTime());boolean isDone=done.contains(key);boolean isToday=key.equals(today);addDayCell(grid,String.valueOf(day),isDone,isToday,key.equals(today),prefs.getInt("day_count_"+key,0));}
