...
public class MainActivity extends Activity {
    ...
    private TextView text(String value, float size, boolean bold) {
        ...
        v.setText(value); v.setTextSize(size); v.setTextColor(Color.rgb(35,40,38));
    ...
    private void addCard(LinearLayout parent, String heading, String body) {
        ...
        card.setPadding(dp(20), dp(16), dp(20), dp(16)); card.setBackground(bg(Color.WHITE, 18));
        TextView h = text(heading, 17, true); h.setTextColor(Color.rgb(25,86,68)); card.addView(h);
        card.addView(text(body, 15.5f, false));
    ...
    private void buildScreen() {
        ...
        root.setPadding(dp(18), dp(16), dp(18), dp(18)); root.setBackgroundColor(Color.rgb(246,247,245));

        LinearLayout header = new LinearLayout(this); header.setOrientation(LinearLayout.VERTICAL);
        TextView title = text("A Jornada da Bíblia", 28, true); title.setTextColor(Color.rgb(22,75,59)); header.addView(title);
        TextView subtitle = text("Sua caminhada pela Palavra, em sequência.", 14, false); subtitle.setTextColor(Color.rgb(90,98,94)); header.addView(subtitle);
        root.addView(header);

        ScrollView scroll = new ScrollView(this); LinearLayout content = new LinearLayout(this);
        content.setOrientation(LinearLayout.VERTICAL); content.setPadding(0,dp(18),0,dp(12));

        LinearLayout readingCard = new LinearLayout(this); readingCard.setOrientation(LinearLayout.VERTICAL);
        readingCard.setPadding(dp(22),dp(20),dp(22),dp(20)); readingCard.setBackground(bg(Color.rgb(221,237,229),22));
        ...
        readingCard.addView(text("Capítulo inteiro  •  17 versículos",14,false));
        readingCard.addView(text("A oferta queimada e a adoração diante de Deus.",14,false));
        ...
        LinearLayout.LayoutParams rb = new LinearLayout.LayoutParams(-1,dp(52)); rb.setMargins(0,dp(14),0,0); readingCard.addView(read,rb);
        content.addView(readingCard);
        ...
        progressRow.addView(text("Seu progresso",17,true),new LinearLayout.LayoutParams(0,-2,1));
        TextView percent = text(done?"100%":"0%",15,true); percent.setTextColor(Color.rgb(24,91,70)); progressRow.addView(percent); progressCard.addView(progressRow);
        ProgressBar progress=new ProgressBar(this,null,android.R.attr.progressBarStyleHorizontal); progress.setMax(100); progress.setProgress(done?100:0); progressCard.addView(progress);
        progressCard.addView(text(done?"Leitura concluída. Muito bem!":"Você está começando sua jornada.",13.5f,false));
        ...
        LinearLayout playerCard = new LinearLayout(this); playerCard.setOrientation(LinearLayout.VERTICAL); playerCard.setPadding(dp(18),dp(16),dp(18),dp(16)); playerCard.setBackground(bg(Color.rgb(232,238,234),18));
        LinearLayout.LayoutParams pl = new LinearLayout.LayoutParams(-1,-2); pl.setMargins(0,dp(2),0,dp(12)); content.addView(playerCard,pl);
        playerCard.addView(text("🔊  Leitor do devocional",17,true));
        playerStatus = text("Pronto para ouvir",13.5f,false); playerStatus.setTextColor(Color.rgb(85,94,89)); playerCard.addView(playerStatus);
        ...
        Button complete=button(done?"✓  Leitura concluída":"✓  Marcar leitura como concluída"); complete.setTextColor(done?Color.rgb(24,91,70):Color.WHITE); complete.setBackground(bg(done?Color.rgb(224,235,228):Color.rgb(24,91,70),16));
        complete.setOnClickListener(v->{prefs.edit().putBoolean("lev1",true).apply(); buildScreen();}); content.addView(complete,new LinearLayout.LayoutParams(-1,dp(52)));

        content.addView(text("Próxima etapa",17,true));
        content.addView(text("Depois de concluir, a jornada avançará para a próxima leitura definida. A divisão dos capítulos será feita conforme o contexto do texto, e não por números aleatórios de versículos.",14,false));
    ...
    private void toggleSpeech() {
        if (tts == null) return;
        if (speaking) {
            tts.stop(); speaking=false; paused=true; playPause.setText("▶  Continuar"); playerStatus.setText("Pausado na parte " + (speechPart + 1) + " de 5");
            return;
        }
        speaking=true; paused=false; playPause.setText("Ⅱ  Pausar"); playerStatus.setText("Reproduzindo parte " + (speechPart + 1) + " de 5"); speakCurrentPart();
    }

    private void speakCurrentPart() {
        if (!speaking || tts == null || speechPart >= speechParts.length) { finishSpeech(); return; }
        tts.speak(speechParts[speechPart], TextToSpeech.QUEUE_FLUSH, null, "devocional-" + speechPart);
        // Android TTS callbacks are not required for this simple reader; the user can pause/stop at any time.
    }

    private void stopSpeech() {
        if (tts != null) tts.stop(); speaking=false; paused=false; speechPart=0;
        if (playPause != null) playPause.setText("▶  Ouvir");
        if (playerStatus != null) playerStatus.setText("Pronto para ouvir");
    }

    private void finishSpeech() {
        speaking=false; paused=false; speechPart=0;
        if (playPause != null) playPause.setText("▶  Ouvir novamente");
        if (playerStatus != null) playerStatus.setText("Devocional concluído");
    }
